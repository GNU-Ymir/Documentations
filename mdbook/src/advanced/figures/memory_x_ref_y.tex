...
\begin{tikzpicture}
  ...
  \draw[Box] ($(LEFT.east)+(0.5, 4)$) -- ($(LEFT.east)+(0.5, 4)$) node[anchor=center]{\textbf{\textit{y}}};
  \draw[line width=1pt] ($(LEFT.east)+(0.3, 4.4)$) rectangle ($(LEFT.east)+(6.8, 3.6)$);
  \draw[Box] ($(LEFT.east)+(3.5, 4)$) -- ($(LEFT.east)+(3.5, 4)$) node[anchor=center]{+0};

  \draw[thick,->,shorten >=1pt] ($(LEFT.east)+(-0.3, 4)$) to [out=180,in=180] node[below left, yshift=3mm] {} ($(LEFT.east)+(-0.7, 6.5)$);
